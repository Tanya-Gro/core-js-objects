...
function makeWord(lettersObject) {
  const result = [];
  Object.entries(lettersObject).forEach(([key, indexArray]) =>
 ...
 */
function sellTickets(queue, d25 = 0, d50 = 0, d100 = 0) {
  if (queue.length === 0) return true;
  const buills = [d25, d50, d100];
  if (queue[0] === 25) {
    buills[0] += 1;
  } else if (queue[0] === 50) {
    if (buills[0] > 0) buills[0] -= 1;
    else return false;
    buills[1] += 1;
  } else {
    if (buills[1] > 0 && buills[0] > 0) {
      buills[0] -= 1;
      buills[1] -= 1;
    } else if (buills[0] >= 3) buills[0] -= 3;
    else return false;
    buills[2] += 1;
  }
  return sellTickets(queue.splice(1), ...buills);
}
...
function group(array, keySelector, valueSelector) {
  return array.reduce((acc, item) => {
    const groupedField = keySelector(item);
    const items = acc.get(groupedField) || [];
    items.push(valueSelector(item));
    acc.set(groupedField, items);
    return acc;
  }, new Map());
}
